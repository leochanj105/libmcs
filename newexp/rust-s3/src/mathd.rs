...
pub fn exp2d(x: f64) -> f64 {
    // For exact integer inputs, construct the result exactly via bit manipulation
    // (matches C pow(2.0, x) behavior for integer arguments)
    let n = x as i64;
    if n as f64 == x && n >= -1074 && n <= 1023 {
        if n >= -1022 {
            // Normal: 2^n has biased exponent (n + 1023), mantissa = 0
            f64::from_bits(((n + 1023) as u64) << 52)
        } else {
            // Subnormal: 2^n = 1 bit at position (n + 1074)
...
pub fn ilogbd(x: f64) -> i32 {
    ...
    if (hx as u32) < 0x00100000 {
        if (hx as u32 | lx) == 0 {
            return -i32::MAX; // FP_ILOGB0 = -2147483647
        } else if hx == 0 {
            let mut ix = -1043i32;
            while (lx as i32) > 0 {
                ix -= 1;
