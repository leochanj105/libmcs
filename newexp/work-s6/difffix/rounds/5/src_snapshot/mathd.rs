...
pub fn rem_pio2(x: f64, y: &mut [f64]) -> i32 {
    ...
    if ix >= 0x7ff00000 {
        let nan = if x.is_nan() {
            x - x  // propagate NaN input
        } else {
            f64::from_bits(0x7ff8_0000_0000_0000)  // positive qNaN for inf input
        };
        y[0] = nan;
        y[1] = nan;
        return 0;
...
pub fn ilogbd(x: f64) -> i32 {
    ...
    if (hx as u32) < 0x00100000 {
        if (hx as u32 | lx) == 0 {
            return i32::MIN + 1; // FP_ILOGB0 = -2147483647
        } else if hx == 0 {
            let mut ix = -1043i32;
            while (lx as i32) > 0 {
                ix -= 1;
...
pub fn powd(x: f64, y: f64) -> f64 {
    ...
    if (iy as u32 | ly) == 0 {
        /* x**0 = 1 unless x is snan */
        let x_is_snan = (ix > 0x7ff00000 || (ix == 0x7ff00000 && lx != 0)) && (ix & 0x00080000) == 0;
        if x_is_snan { return x + y; }
        return 1.0;
    }

    if ix > 0x7ff00000 || (ix == 0x7ff00000 && lx != 0)
    || iy > 0x7ff00000 || (iy == 0x7ff00000 && ly != 0) {
        let y_is_snan = (iy > 0x7ff00000 || (iy == 0x7ff00000 && ly != 0)) && (iy & 0x00080000) == 0;
        if ((hx - 0x3ff00000) as u32 | lx) == 0 && !y_is_snan { return 1.0; }
        return x + y;
    }
