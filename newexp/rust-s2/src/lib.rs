...
#[no_mangle] pub extern "C" fn feclearexcept(_excepts: i32) -> i32 { 0 }
#[no_mangle] pub extern "C" fn feraiseexcept(_excepts: i32) -> i32 { 0 }
#[no_mangle] pub unsafe extern "C" fn fegetexceptflag(flagp: *mut FexceptT, _excepts: i32) -> i32 { -1 }
#[no_mangle] pub unsafe extern "C" fn fesetexceptflag(flagp: *const FexceptT, _excepts: i32) -> i32 { -1 }
#[no_mangle] pub extern "C" fn fegetround() -> i32 { 0 }
#[no_mangle] pub extern "C" fn fesetround(rdir: i32) -> i32 { if rdir == 0 { 0 } else { -1 } }
...
#[no_mangle] pub extern "C" fn fetestexcept(_excepts: i32) -> i32 { 0 }

// ─── signgam global ─────────────────────────────────────────────────────────
// In C, `signgam` is a macro for `__signgam` (see libm/include/math.h).
// Tests link against `__signgam` directly, so we must export that symbol.
...
#[no_mangle] pub extern "C" fn lgamma(x: f64) -> f64 {
    ...
    unsafe { __signgam = mathd::SIGNGAM.with(|s| s.get()); }
    r
}
...
// nexttoward: long double is 80-bit on x86-64; C shim in long_double_shim.c
// handles the ABI conversion and calls this internal implementation.
#[no_mangle] pub extern "C" fn __nexttoward_impl(x: f64, y: f64) -> f64 { mathd::nexttowardd(x, y) }
...
#[no_mangle] pub extern "C" fn lgammaf(x: f32) -> f32 {
    ...
    unsafe { __signgam = mathf::SIGNGAM.with(|s| s.get()); }
    r
}
...
// nexttowardf: long double is 80-bit on x86-64; C shim handles ABI conversion
#[no_mangle] pub extern "C" fn __nexttowardf_impl(x: f32, y: f64) -> f32 { mathf::nexttowardf(x, y) }
