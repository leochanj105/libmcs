...
// ─── signgam global ─────────────────────────────────────────────────────────
// Note: thread-local in Rust; exposed as a readable static for compatibility.
...
#[no_mangle] pub extern "C" fn lgamma(x: f64) -> f64 {
    ...
    unsafe { signgam = mathd::SIGNGAM.with(|s| s.get()); }
    r
}
...
#[no_mangle] pub extern "C" fn lgammaf(x: f32) -> f32 {
    ...
    unsafe { signgam = mathf::SIGNGAM.with(|s| s.get()); }
    r
}
