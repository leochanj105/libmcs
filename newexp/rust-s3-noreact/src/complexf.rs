...
pub fn casinf(z: FloatComplex) -> FloatComplex {
    ...
    let zz = FloatComplex { re: ct.re + z2.re, im: ct.im + z2.im };
    let zz = clogf(zz);
    // w = zz * (-I)
...
pub fn cacosf(z: FloatComplex) -> FloatComplex {
    ...
    let tmp1 = if tmp1.is_nan() { -tmp1 } else { tmp1 };
...
pub fn casinhf(z: FloatComplex) -> FloatComplex {
    ...
    let tmp = casinf(tmp);
    let im = -tmp.re;
    let im = if im.is_nan() { f32::from_bits(im.to_bits() & !(1u32 << 31)) } else { im };
    FloatComplex { re: tmp.im, im }
}
